...
env GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o dist/windows-amd64/ -ldflags="-X 'main.Version=${version}'"
if [ $? -ne 0 ]; then
...
env GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o dist/linux-amd64/ -ldflags="-X 'main.Version=${version}'"
if [ $? -ne 0 ]; then
...
env GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -o dist/darwin-amd64/ -ldflags="-X 'main.Version=${version}'"
if [ $? -ne 0 ]; then
...
env GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -o dist/darwin-arm64/ -ldflags="-X 'main.Version=${version}'"
if [ $? -ne 0 ]; then
